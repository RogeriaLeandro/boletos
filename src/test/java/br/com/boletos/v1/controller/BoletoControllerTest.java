...
public class BoletoControllerTest {
    ...
    void consultarBoletoPagoPorDocumento(){
        ...
        boleto.setUuidAssociado(BoletoTestHelper.ID_ASSOCIADO);
        var optionalBoleto = Optional.of(boleto);
        doReturn(optionalBoleto).when(boletoService).consultarBoletoPorUuidEPorSituacao(BoletoTestHelper.ID_ASSOCIADO, SituacaoBoleto.PAGO.getDescricaoSituacaoBoleto());
        var response = target.consultaBoletoPagoPorUUIDAssociado(BoletoTestHelper.ID_ASSOCIADO);
        assertEquals(boleto, response.getBody());
        assertEquals(HttpStatus.OK, response.getStatusCode());
        verify(boletoService).consultarBoletoPorUuidEPorSituacao(BoletoTestHelper.ID_ASSOCIADO, SituacaoBoleto.PAGO.getDescricaoSituacaoBoleto());
    }

    @Test
    void nenhumBoletoPagoEncontradoAoConsultarPorAssociado(){
        doReturn(Optional.empty()).when(boletoService).consultarBoletoPorUuidEPorSituacao(BoletoTestHelper.ID_ASSOCIADO, SituacaoBoleto.PAGO.getDescricaoSituacaoBoleto());
        var response = target.consultaBoletoPagoPorUUIDAssociado(BoletoTestHelper.ID_ASSOCIADO);
        assertEquals(null, response.getBody());
        assertEquals(HttpStatus.NO_CONTENT, response.getStatusCode());
        verify(boletoService).consultarBoletoPorUuidEPorSituacao(BoletoTestHelper.ID_ASSOCIADO, SituacaoBoleto.PAGO.getDescricaoSituacaoBoleto());
    ...
    void consultarBoletoEmAbertoPorDocumento(){
        ...
        boleto.setUuidAssociado(BoletoTestHelper.ID_ASSOCIADO);
        var optionalBoleto = Optional.of(boleto);
        doReturn(optionalBoleto).when(boletoService).consultarBoletoPorUuidEPorSituacao(BoletoTestHelper.ID_ASSOCIADO, SituacaoBoleto.EM_ABERTO.getDescricaoSituacaoBoleto());
        var response = target.consultaBoletoEmAbertoPorUUIDAssociado(BoletoTestHelper.ID_ASSOCIADO);
        assertEquals(boleto, response.getBody());
        assertEquals(HttpStatus.OK, response.getStatusCode());
        verify(boletoService).consultarBoletoPorUuidEPorSituacao(BoletoTestHelper.ID_ASSOCIADO, SituacaoBoleto.EM_ABERTO.getDescricaoSituacaoBoleto());
    }

    @Test
    void nenhumBoletoEmAbertoEncontradoAoConsultarPorAssociado(){
        doReturn(Optional.empty()).when(boletoService).consultarBoletoPorUuidEPorSituacao(BoletoTestHelper.ID_ASSOCIADO, SituacaoBoleto.EM_ABERTO.getDescricaoSituacaoBoleto());
        var response = target.consultaBoletoEmAbertoPorUUIDAssociado(BoletoTestHelper.ID_ASSOCIADO);
        assertEquals(null, response.getBody());
        assertEquals(HttpStatus.NO_CONTENT, response.getStatusCode());
        verify(boletoService).consultarBoletoPorUuidEPorSituacao(BoletoTestHelper.ID_ASSOCIADO, SituacaoBoleto.EM_ABERTO.getDescricaoSituacaoBoleto());
